...
> 本汇编面向安全研究与渗透测试人员，**全部技术仅用于授权测试（authorized penetration testing / security assessments）**，请确保已获得目标系统所有者的书面授权，严禁用于未授权的非法活动。

Web Application Firewall (WAF) is a security tool that protects web applications from various attacks by analyzing HTTP requests and applying rules to identify and block suspicious traffic. This document outlines effective techniques to bypass WAF protections during security assessments.
...
Services like ZenRows implement sophisticated anti-bot techniques, including premium proxies, JS rendering, WAF bypassing capabilities and CAPTCHA solving. (Commercially licensed usage only.)
...
- Check historical DNS records (e.g., with the `securitytrails` API) — 2024 research found ~40% of Fortune-100 origins exposed via stale A records.
...
- **Cloudscraper** - Python library to avoid the Cloudflare waiting room.
...
- **Cloudflare Turnstile** - low-friction CAPTCHA replacement; see the `cloudflare_turnstile_bypass` PoC (GUI-driven, results may vary).
...
- Randomize both JA3 and JA4 fingerprints using libraries such as `noble-tls` or `ja4py`.
...
- **SQLMAP tamper scripts**:
  - Use the Atlas tool for suggesting tamper scripts.
...
- **Request Header Manipulation**:
  ...
  - Adding headers that make the request appear to come from internal networks or spoof identity. Many applications, especially behind misconfigured reverse proxies, trust headers like `X-Forwarded-For` or `X-Forwarded-Host` to determine the client's IP or the requested host. Manipulating these can lead to various vulnerabilities:
    ...
    - **SSRF**: Headers like `X-Forwarded-For` or `X-Real-IP` can be manipulated to target internal IPs (e.g., `169.254.169.254` for the AWS metadata service).
...
- **HTTP RFC Inconsistencies**:
  ...
  - Exploiting differences in how servers handle the HTTP specification.
...
- **Host Header Spoofing**:
  ...
  - **Mechanism**:
    ...
    1. Client connects to `evil.example.com` (e.g., the IP address of `evil.example.com`).
  ...
  - **Protection**:
    ...
    - This check prevents the simple Host Header Spoofing described above.
...
- **Advanced Encoding Techniques**:
  ...
  - **Double Unicode Escaping**: `\u0061` for character '1'.
